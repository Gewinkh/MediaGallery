...
void PdfExtractController::extractOrdered(const QVariantList& items,
                                          const QString& folderOrUrl,
                                          const QString& baseName) {
    ...
    for (const QVariant& iv : items) {
        ...
        const QString path = mg::toLocalPath(m.value(QStringLiteral("path")).toString());
        bool ok = false;
        const int page = m.value(QStringLiteral("page")).toInt(&ok);
        if (path.isEmpty() || !ok || page < 0 || !QFileInfo::exists(path))
            continue;
        const QString key = path + QLatin1Char('\n') + QString::number(page);
        if (seen.contains(key)) continue;
        seen.insert(key);
        if (firstPath.isEmpty()) firstPath = path;
        if (!jobs.isEmpty() && jobs.last().path == path)
            jobs.last().pages.append(page);
        else
    ...
    if (jobs.isEmpty() || folder.isEmpty() || !QFileInfo(folder).isDir()) {
        emit extractFinished(false, QString(), QStringLiteral("input"));
        return;
    }
