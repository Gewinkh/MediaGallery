...
void PdfEditController::reflowChain(int anyId, int editedId,
    ...
    for (int i = 0; i < chain.size(); ++i) {
        ...
        if (i == chain.size() - 1) {
            ...
            const int fit = fitCharCount(*b, combined.mid(pos));
...
void PdfEditController::exportContentEdited() {
    ...
    if (eligible) {
        const QVector<PdfEditBox> boxes = m_model.boxes();
        for (const PdfEditBox& b : boxes) {
            if (b.kind != PdfAnnKind::Replace || b.origText.isEmpty()) { eligible = false; break; }
    ...
    if (eligible) {
        ...
        if (QFileInfo(pristinePath()).size() > kMaxContentEditBytes)
            eligible = false;
    }
    ...
    if (eligible) {
        m_pool.start(new PdfContentEditTask(this, pristinePath(), exportTargetPath(), edits, gen));
    } else {
        // Direkt Raster-Export (Fallback) — immer korrekt, inkl. aller Notizen.
        emit contentEditFellBack();
        m_cancel = std::make_shared<std::atomic<bool>>(false);
        m_pool.start(new PdfExportTask(this, pristinePath(), exportTargetPath(),
                                       m_model.boxes(), m_plan, gen, m_cancel));
    }
}
...
void PdfEditController::contentEditTaskFinished(bool ok, const QString& target,
    ...
    m_cancel = std::make_shared<std::atomic<bool>>(false);
    m_pool.start(new PdfExportTask(this, pristinePath(), exportTargetPath(),
                                   m_model.boxes(), m_plan, m_exportGen, m_cancel));
}
