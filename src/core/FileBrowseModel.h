#pragma once

// Daten fuer den eigenen Dateiwaehler: Qts FileDialog steckt in eigener QML, dort ist
// weder SmoothWheelArea einhaengbar noch das Aussehen bestimmbar. Gelesen wird
// asynchron - auf einem Netzlaufwerk blockiert schon das Auflisten spuerbar.

#include <QAbstractListModel>
#include <QDateTime>
#include <QStringList>
#include <QThreadPool>
#include <QUrl>

#include <atomic>
#include <functional>
#include <vector>
#include <memory>

class FileBrowseModel : public QAbstractListModel {
    Q_OBJECT
    Q_PROPERTY(QString folder READ folder WRITE setFolder NOTIFY folderChanged)
    Q_PROPERTY(QStringList nameFilters READ nameFilters WRITE setNameFilters
                   NOTIFY nameFiltersChanged)
    Q_PROPERTY(bool showHidden READ showHidden WRITE setShowHidden
                   NOTIFY showHiddenChanged)
    // Begleitdateien der App mitzeigen? Gebunden an `App.showAllFiles` - dieselbe Regel wie in der Galerie
    // (`mg::isCompanionFile`), damit dieselbe Datei nicht hier auftaucht und dort fehlt.
    Q_PROPERTY(bool showAllFiles READ showAllFiles WRITE setShowAllFiles
                   NOTIFY showAllFilesChanged)
    Q_PROPERTY(bool dirsOnly READ dirsOnly WRITE setDirsOnly NOTIFY dirsOnlyChanged)
    Q_PROPERTY(int  count READ count NOTIFY countChanged)
    Q_PROPERTY(bool loading READ loading NOTIFY loadingChanged)
    Q_PROPERTY(bool canGoUp READ canGoUp NOTIFY folderChanged)
    Q_PROPERTY(int  sortKey READ sortKey WRITE setSortKey NOTIFY sortChanged)
    Q_PROPERTY(bool sortDescending READ sortDescending WRITE setSortDescending
                   NOTIFY sortChanged)

public:
    explicit FileBrowseModel(QObject* parent = nullptr);
    ~FileBrowseModel() override;

    enum SortKey { SortName = 0, SortSize = 1, SortDate = 2 };
    Q_ENUM(SortKey)

    struct Row {
        QString   name;
        bool      isDir = false;
        qint64    size  = 0;
        QDateTime mtime;
    };

    enum Roles {
        NameRole = Qt::UserRole + 1,
        IsDirRole,
        PathRole,
        SizeTextRole,
        DateTextRole
    };

    int      rowCount(const QModelIndex& parent = {}) const override;
    QVariant data(const QModelIndex& index, int role) const override;
    QHash<int, QByteArray> roleNames() const override;

    QString     folder() const { return m_folder; }
    void        setFolder(const QString& path);
    QStringList nameFilters() const { return m_filters; }
    void        setNameFilters(const QStringList& f);
    bool        showHidden() const { return m_showHidden; }
    bool        showAllFiles() const { return m_showAllFiles; }
    void        setShowAllFiles(bool v);
    void        setShowHidden(bool v);
    bool        dirsOnly() const { return m_dirsOnly; }
    void        setDirsOnly(bool v);
    int         count() const { return int(m_rows.size()); }
    bool        loading() const { return m_loading; }
    bool        canGoUp() const;

    Q_INVOKABLE void cdUp();
    Q_INVOKABLE void reload();
    Q_INVOKABLE QString entryPath(int row) const;
    Q_INVOKABLE bool    entryIsDir(int row) const;

    Q_INVOKABLE QVariantList places() const;
    Q_INVOKABLE QVariantList crumbs() const;
    Q_INVOKABLE QString  join(const QString& dir, const QString& name) const;
    Q_INVOKABLE QString  dirOf(const QString& path) const;
    Q_INVOKABLE QString  baseName(const QString& path) const;
    Q_INVOKABLE bool     fileExists(const QString& path) const;
    Q_INVOKABLE bool     dirExists(const QString& path) const;
    // Liefert 0 = angelegt, 1 = Name unbrauchbar, 2 = gibt es schon, 3 = fehlgeschlagen. Die Meldung dazu wählt die
    // Oberfläche - das Modell kennt keine Sprache.
    Q_INVOKABLE int      createFolder(const QString& name);
    int  sortKey() const { return m_sortKey; }
    void setSortKey(int k);
    bool sortDescending() const { return m_sortDesc; }
    void setSortDescending(bool d);
    Q_INVOKABLE QString  withSuffix(const QString& name, const QString& suffix) const;
    Q_INVOKABLE QUrl     toUrl(const QString& path) const;
    Q_INVOKABLE QString  fromUrl(const QUrl& url) const;
    Q_INVOKABLE QStringList globsOf(const QString& filterText) const;

signals:
    void folderChanged();
    void sortChanged();
    void nameFiltersChanged();
    void showHiddenChanged();
    void showAllFilesChanged();
    void dirsOnlyChanged();
    void countChanged();
    void loadingChanged();

private:
    void startLoad();
    void applyRows(std::vector<Row> rows, quint64 gen);
    void sortRows(std::vector<Row>& rows) const;

    QString     m_folder;
    QStringList m_filters;
    bool        m_showHidden = false;
    bool        m_showAllFiles = false;
    bool        m_dirsOnly   = false;
    bool        m_loading    = false;
    int         m_sortKey    = SortName;
    bool        m_sortDesc   = false;
    std::vector<Row> m_rows;

    //  Ein Worker genügt - zwei Verzeichnisse gleichzeitig braucht niemand,
    //  und seriell bleibt die Reihenfolge der Ergebnisse eindeutig.
    QThreadPool m_pool;
    quint64     m_gen = 0;                       // verwirft veraltete Läufe
    std::shared_ptr<std::atomic<bool>> m_cancel; // kooperativer Abbruch
};
