...
- **Drop a file on a tag to assign it**: drag a gallery tile onto a tag in the right-hand panel and the file gets that tag. Dropping only ever **adds** - it never removes a tag that is already there. The same drop area also accepts files dragged in from outside the app
