...
- **Footnotes**: a footnote reference shows as a superscript number, and the note itself is set at the foot of its page, under a separator line - both in the editor and in the exported PDF. The footnote text is editable: switch the editing region to Footnotes, just like headers and footers. In the PDF every page reserves the same room for notes, so a document whose footnotes are spread unevenly can come out a page or two longer than the editor shows
- **Tracked changes are shown**: text inserted with Word's change tracking appears underlined, deleted text struck through, each in its author's colour. Deleted text used to be missing from the editor entirely. Right-click a change to accept or reject it - one undo step, and the markup of everything you do not touch stays byte-identical. Recording new changes while you type is not available
...
| DOCX: leave header/footer editing | `Esc` |
